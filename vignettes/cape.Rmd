...
Pleiotropy, or `cape`. This package implements a method, originally described
...
method takes advantage of complementary information in partially pleiotropic genetic 
variants to resolve directional influences between variants that interact 
epistatically. `cape` can be applied to a variety of genetic variants, such as 
single nucleotide polymorphisms (SNPs), copy number variations (CNVs) or structural
variations (SVs). Here we demonstrate the functionality of `cape` by inferring a 
predictive network between quantitative trait loci (QTL) in a cross between the 
non-obese, non-diabetic (NON) mouse and the New Zealand obese (NZO) mouse 
(@Reifsnyder:2000is).

## Installing and Loading cape
CAPE is housed at (github website).
To install...
...
contains all traits and genotypes in the same comma-delimited file.
The first few columns contain traits, covariates, and indvidual identifiers,
and the remaining contain. Please see the qtl demo data included with cape 
for an example. Also see `?read.population` for more details. 

To read in data with this format, we use `read.population`, followed
...
Many genetic mappers are now using the updated [R/qtl2](https://kbroman.org/qtl2/)
...
R/qtl2 is the definitive genetic mapping package, and the 
outstanding documentation can answer many basic questions about genetic mapping.  
...
For cape, we need a ped file, a map file, and a phenotype file. PLINK 
standard format includes only one trait, and cape requires at least two
traits. We therefore require the phenotype file in the pheno.txt format
...
This data set was established to find quantitative trait loci (QTL) for 
...
called ``mom'' indicating whether the mother of each mouse was normal 
weight (0) or obese (1). 

These data can be accessed either through the demo_qtl demonstration code 
or by typing `data(obesity.cross)`

## Data visualization
Before proceeding with an analysis we recommend taking a good look at your data.
...
the data set by using the `pheno.which` argument. Leaving this argument
...
looking at the Qnorm plots for each trait using `qnormPheno.'

```{r qnormPheno, fig.width = 7, fig.height = 3}
...
function `norm.pheno` uses quantile normalization to fit the 
...
Cape relies on the selection of two or more phenotypes that have common 
...
we have also colored the points by the covariate "mom" to look for some
...
gives numeric information about pairwise correlations. If `color.by`
is not specified, only the overall pearson R values are shown for each
trait pair. If `color.by` is specified, we show the overall correlation
as well as correlations for each group in `color.by.`
...
with the cape package. It it probably easiest to start with one of these and
...
as an interactive covariate. Here we specify "mom" as our covariate.

**scan.what:** This parameter refers to whether we will scan individual traits
...
The SVD de-correlates the phenotypes concentrating phenotypic features into 
individual eigentraits. One benefit of this process is that variants that are 
weakly correlated to several phenotypes due to common underlying processes may 
...
between phenotypes easier to detect and identify as potential interaction loci 
and/or covariates.

To specify using individual traits, set `scan.what` to raw.traits.
To specify using eigentraits, set `scan.what` to eigentraits.
...
may which to analyze only a subset of them in cape. For example, if the first 
...
**marker_selection_method:** This parameter should be either "top.effects", or
"from.list." If "top.effects," cape will select markers for the pair scan based
...
in roughly 24 hours using 2 or 3 traits/eigentraits and 1.5 million permuted tests.
Increasing the number of traits/eigentraits analyzed substantially increases 
the time of a run, and it might be necessary to lower the number of markers tested
further if there are many traits/eigentraits in the analysis.
...
**snp_file:** If "marker_selection_method" is "from.list" a file name needs
...
is A, all markers will just have an "_B" appended to their name. If, however, 
this is a multi-parent population, the allele has a real effect. Any of the 
alleles other than the reference can be specified for each marker. 
...
"marker_selection_method" to "top.effects." 
...
by the user as "max_pair_cor". We generally use a value of 0.5 in our own work, but this can
be raised or lowered depending on your own assessment of the influence of
LD in your population.
...
**max_pair_cor:** A numeric value between 0 and 1 indicating the maximum
   ...
   the pair is not tested. If this value is set to NULL, "min.per.genotype" 
...
**min_per_geno:** This is an alternative way to limit the effect of LD
   ...
   will be too high. If this value is NULL, max.pair.cor must have a numeric 
   value.

**pairscan_null_size:** This parameter is a little confusing, as it is 
...
`run.cape()`. Now that we've done the hard work of reading in our data
...
demo NON/NZO mouse backcross. If you run this line interactive, set verbose
...
benefit of the increased ease of use will far outweigh the decrease in flexibility.

If you are in need of more flexibility, all functions in the function 
`run.cape` are exported, so the entire analysis can be run step-by-step 
as it was before. All plotting functions are also exported to allow 
greater flexibility in plotting. Although `run.cape` does some plotting 
automatically, you may wish to re-run some plotting functions with your 
own settings. 
...
decompose the trait matrix into eigentraits. This is done if "scan.what"
...
Cape uses the function `plotSVD` to plot the results of the SVD. The 
plot for the NON/NZO data set are shown below.

```{r eigentraits, results = "asis", echo = FALSE, out.width = 4, out.height = 4}
...
### Single-locus scan
Cape then goes on to perform marker regression on all individual markers:

$$U_{i}^{j} = \beta_{0}^{j} + x_{i}\beta^{j} + \epsilon_{i}^{j}$$

The index $i$ runs from 1 to the number of individuals, and $j$ runs from 
1 to the number of eigentraits or phenotypes. $x_{i}$ is the probability 
...
Although `run.cape` creates files with the singlescan results automatically,
...
results file and using `plotSinglescan` with parameters different from
those in `run.cape.`

```{r single_plot, fig.width = 7, fig.height = 5}
...
between variants. The epistatic models are then combined across phenotypes or 
eigentraits to infer a parsimonious network that takes data from all eigentraits 
into account.

To find epistatic interactions wentests the following regression model for each 
variant 1 and 2:
...
leave-one-chromosome-out method already commonly used, but our results 
suggested that the pairwise additive assumption in this model was not 
appropriate for correcting for kinship effects in an epistatic model.
...
```{r reparam_fig, results = "asis", echo = FALSE, fig.width = 3, fig.height = 3}
...
This inversion is exact for two eigentraits, and `cape` implements pseudo-inversion 
for up to 12 eigentraits. We have observed that maximum power to detect interactions
is achieved with three to five eigentraits (@tyler2017epistatic). 
...
of as the direct influence of marker 2 on marker 1, with negative values indicating 
repression and positive values indicating enhancement. The terms $m_{12}$ and 
$m_{21}$ are self-consistent and defined in terms of $\delta_1$ and $\delta_2$:
...
other marker independent of phenotype. The significance of these influences is 
determined through standard error analysis on the regression parameters 
(@bevington1994data, @Carter:2012fd). This step is particularly important as matrix 
...
function `writeVariantInfluenes` for a description of the columns
in these files. 

Cape results can also be plotted in multiple different forms. The function
`plotVariantInfluences` plots the results as a heatmap. Interactions
...
```{r var_inf, results = "asis", echo = FALSE}
...
The function `plotNetwork` plots cape results as a circular network.
...
```{r circ_net, results = "asis", echo = FALSE}
circ.net.file <- here("demo", "demo_qtl", "Network.Circular.jpg")
cat(paste0("![](", circ.net.file, ")\n"))
```

And finally, the function `plotFullNetwork.` This function plots
...
```{r net_vis, results = "asis", echo = FALSE}
...
new plotting function in cape called `plot.effects` that can be used
...
`ploEffects`. First to verify the positive effect of the marker on Chr 15,
we can plot it by itself. Here we use `plot.type = l` which generates a line
plot.

```{r plot_main, fig.width = 7, fig.height = 3}
...
at different types of interactions, including heat maps, and points for individual
values. 
...
**histPheno:** Plots trait distributions as histograms. Helps identify
whether traits are normally distributed.

**qnormPheno:** Plots trait quantiles compared to theoretical normal 
quantiles. Helps identify whether traits are normally distributed.

**plotPhenoCor:** Plots the correlation between pairs of traits. 
...
**plotSVD:** Plots the results of the singular value decomposition
...
**plotSinglescan:** Plots the results of the single-locus scan.
Can plot either standardized effects or undstandardized effects.
Also plots effects both by allele and by marker.

**plotPairscan:** Plots the results of the pairwise scan.

**plotVariantInfluences:** Plots cape results as a heatmap. Genetic
...
**plotNetwork:** Plots cape results as a circular network. Genetic
...
**plotFullNetwork:** Plots cape results in a standard network layout.
...
**plotEffects:** Plots the effects of individual interactions on traits.
...
plotVariantInfluences.csv or Variant.Influences.Interactions.csv. The 
function plotVariantInfluences can also return these tables invisibly 
without writing to a file. 

## List of Output Files
The following list describes each output file produced by `run.cape()`.
...
The results are a list of the following seven elements:
   ...
   * alpha.thresh: The calculated effect size thresholds at each 
      alpha if permutations are run.
   * ref.allele: The allele used as the reference allele
   * singlescan.effects: The effect sizes (beta coefficients) from the 
...
name will be included in the filename. This plot is generated by `plotSinglescan`.

**Singlescan.ET1.Standardized.jpg:** The results of the single-locus scan. 
In contrast to the file above, this plot shows only the standardized effects
for each locus. Similar to the file above, there will be one effects plot 
for each trait, and the trait name will be included in the filename. 
This plot is generated by `plotSinglescan`.
...
is generated by `plotSVD`. It shows the total trait variance 
explained by each eigentrait, as well as the contribution of 
each trait to each eigentrait.
...
and written by the function `writeVariantInfluences.' This file contains
