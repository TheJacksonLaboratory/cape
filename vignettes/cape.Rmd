...
with cape for an example. Also see `?read.population` for more details. 
...
to allow greater flexibility in plotting. Although `run.cape` does some 
...
### Single-locus scan
After computing eigentrats, we go on to perform marker regression on all 
individual markers:
...
eigentraits to infer a parsimonious network that takes data from all 
