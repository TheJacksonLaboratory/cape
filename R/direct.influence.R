...
#' 
#' This function recasts the variant-to-eigentrait effects in terms of variant-to-phenotype 
...
#' @param perm.data 
#' @param save.permutations A logical value indicating whether the data from permutations should be saved. Saving the permutations requires more memory but can be helpful in diagnostics. If save.permutations is TRUE all permutation data are saved in an object called "permutation.data.RData".
#' @param n.cores 
#' @param path 
#' @param verbose
#' 
#' @return This function adds a total of six objects to the data object. First, a flag 
#' (\code{transform.to.phenospace}) is added to the data object to indicate whether 
#' variant influences were transformed to phenotype space. The results from the pairwise 
#' scan and the permutations of the pairwise scan are converted to phenospace if specified. 
#' These actions each add one object each to the data object (\code{var.to.pheno.influence} 
#' and \code{var.to.pheno.influence.perm}). Each element is itself a list of matrices 
#' corresponding to the original phenotypes. Each matrix contains one row per marker 
#' pair (or permutation of a marker pair) and contains the influence coefficient and 
#' standard error of the influence coefficient for each pair. After the coefficients 
#' have been transformed to phenotype space, each marker is considered individually 
#' and its influence on each phenotype across all marker pair contexts is tabulated. 
#' This is done for both the pairwise scan and the permutations of the pairwise scans 
#' and adds two new objects (\code{var.to.pheno.test.stat} and \code{var.to.pheno.test.stat.perm}) 
#' to the data object. Each object is a list containing one element for each of the original 
#' phenotypes. Each element contains a table in which all instances of each marker are 
#' listed along with that marker's direct phenotypic influence, the standard error of the 
#' influence, and the t statistic (\eqn{\beta/\sigma}) of the influence. Finally, because 
#' each marker can only have one influence on each phenotype, the influences from each 
#' marker pair context are filtered to report only the maximum influence of each marker 
#' on each phenotype across all marker pair contexts. This process adds an object to the 
#' data object called \code{max.var.to.pheno.influence}. This object is a list containing 
#' one element per phenotype. It tabulates the maximum influence of each marker on each 
#' phenotype, as well as the empirical and Holm's corrected p values associated with each 
#' influence.
#' 
