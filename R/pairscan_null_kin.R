  ...
  while(final_perm < pairscan_null_size){
    ...
    if(marker_selection_method != "by_gene" && marker_selection_method != "from_list"){
      ...
      if(marker_selection_method == "uniform"){
        perm_data_obj <- select_markers_for_pairscan_uniform(perm_data_obj, geno_obj, 
        num_alleles = ncol(data_obj$geno_for_pairscan), verbose = verbose)	
      }
      if(marker_selection_method == "effects_dist"){
        perm_data_obj <- select_markers_for_pairscan_dist(perm_data_obj, singlescan_obj = single_scan_result, 
        geno_obj, verbose = verbose)		
      }
    }else{ 
