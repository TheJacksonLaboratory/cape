myImagePlot <-
  function(x,...){
    ...
    allele.cols <- additional.arguments$allele.cols
    additional.arguments$allele.cols <- NULL
    ...
    if(!is.null(allele.cols)){
      par(xpd = TRUE)
      for(i in 1:(length(chromosome.coordinates)-1)){
        text(x = 1:dim(x)[1], y = poly.min*(1.5+poly.perc), labels = rep("|", dim(x)[1]), col = allele.cols)
        # text(y = 1:dim(x)[1], x = poly.min*(1.2+poly.perc), labels = rep("|", dim(x)[1]), col = allele.cols)
      }
      par(xpd = FALSE)
    }
    ...
    if(!is.null(mark.coords)){
      points(mark.coords[,1], mark.coords[,2], col = mark.col, pch = 16, cex = 0.5)
    }
