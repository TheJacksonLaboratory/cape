...
#' @param pt.cex A numeric value indicating the size of the points.
#' 
#' @export
...
plotPhenoCor <- function(data.obj, pheno.which = NULL, color.by = NULL, group.labels = NULL,
  text.cex = 1, pheno.labels = NULL, pt.cex = 1){
	...
	sub.pheno <- all.pheno[,match(pheno.names, colnames(all.pheno))]
	...
	if(!is.null(color.by)){
		...
		cols <- rep("black", dim(sub.pheno)[1])
		groups <- NULL
		}
	...
	plot.cor <- function(x,y){
		points(x,y, col = cols, cex = pt.cex, pch = 16)
		if(!is.null(color.by)){
	...
	pairs(sub.pheno, lower.panel = plot.cor, upper.panel = write.cor, diag.panel = panel.hist, labels = pheno.labels)
